// https://github.com/mikbry/material-ui-color/blob/master/translations/frFR.json

export const frFR = {
  transparent: 'transparent',
  none: 'invalide',

  aliceblue: 'bleu-alice',
  antiquewhite: 'blanc antique',
  aqua: 'eau',
  aquamarine: 'bleu-marine',
  azure: 'azur',
  beige: 'beige',
  bisque: 'bisque',
  black: 'noir',
  blanchedalmond: "lait d'amande",
  blue: 'bleu',
  blueviolet: 'bleu-violet',
  brown: 'brun',
  burlywood: 'bois rustique',
  cadetblue: 'bleu cadet',
  chartreuse: 'chartreuse',
  chocolate: 'chocolat',
  coral: 'corail',
  cornflowerblue: 'bleuet',
  cornsilk: 'barbe de maïs',
  crimson: 'cramoisi',
  cyan: 'cyan',
  darkblue: 'bleu foncé',
  darkcyan: 'cyan foncé',
  darkgoldenrod: 'jaune doré foncé',
  darkgray: 'gris foncé',
  darkgrey: 'gris foncé',
  darkgreen: 'vert foncé',
  darkkhaki: 'kaki foncé',
  darkmagenta: 'magenta foncé',
  darkolivegreen: 'vert olive foncé',
  darkorange: 'orange foncé',
  darkorchid: 'orchidée foncé',
  darkred: 'rouge foncé',
  darksalmon: 'saumon foncé',
  darkseagreen: "vert d'eau foncé",
  darkslateblue: 'bleu ardoise foncé',
  darkslategray: 'gris ardoise foncé',
  darkslategrey: 'gris ardoise foncé',
  darkturquoise: 'turquoise foncé',
  darkviolet: 'violet foncé',
  deeppink: 'rose profond',
  deepskyblue: 'bleu-ciel profond',
  dimgray: 'gris mat',
  dimgrey: 'gris mat',
  dodgerblue: 'cyan-bleu',
  firebrick: 'rouge brique',
  floralwhite: 'blanc floral',
  forestgreen: 'vert forêt',
  fuchsia: 'fuchsia',
  gainsboro: 'gris étain',
  ghostwhite: 'blanc spectral',
  gold: 'or',
  goldenrod: 'jaune doré',
  gray: 'gris',
  grey: 'gris',
  green: 'vert',
  greenyellow: 'vert-jaune',
  honeydew: 'miellat',
  hotpink: 'rose vif',
  indianred: 'rouge indien',
  indigo: 'indigo',
  ivory: 'ivoire',
  khaki: 'kaki',
  lavender: 'lavande',
  lavenderblush: 'blanc lavande',
  lawngreen: 'vert gazon',
  lemonchiffon: 'mousseline de citron',
  lightblue: 'bleu clair',
  lightcoral: 'corail clair',
  lightcyan: 'cyan clair',
  lightgoldenrodyellow: 'jaune doré clair',
  lightgray: 'gris clair',
  lightgrey: 'gris clair',
  lightgreen: 'vert clair',
  lightpink: 'rose clair',
  lightsalmon: 'saumon clair',
  lightseagreen: "vert d'eau clair",
  lightskyblue: 'bleu-ciel clair',
  lightslategray: 'gris ardoise clair',
  lightslategrey: 'gris ardoise clair',
  lightsteelblue: 'bleu acier clair',
  lightyellow: 'jaune clair',
  lime: 'citron vert',
  limegreen: 'citron vert foncé',
  linen: 'lin',
  magenta: 'magenta',
  maroon: 'marron',
  mediumaquamarine: 'bleu-marine moyen',
  mediumblue: 'bleu moyen',
  mediumorchid: 'orchidée moyen',
  mediumpurple: 'pourpre',
  mediumseagreen: "vert d'eau moyen",
  mediumslateblue: 'bleu ardoise moyen',
  mediumspringgreen: 'vert printemps moyen',
  mediumturquoise: 'turquoise moyen',
  mediumvioletred: 'rouge violet moyen',
  midnightblue: 'bleu nuit',
  mintcream: 'menthe crème',
  mistyrose: 'rose brumeux',
  moccasin: 'mocassin',
  navajowhite: 'blanc navajo',
  navy: 'marine',
  oldlace: 'blanc dentelle',
  olive: 'olive',
  olivedrab: 'olive pâle',
  orange: 'orange',
  orangered: 'rouge orangé',
  orchid: 'orchidée',
  palegoldenrod: 'jaune doré pâle',
  palegreen: 'vert pâle',
  paleturquoise: 'turquoise pâle',
  palevioletred: 'rouge orangé pâle',
  papayawhip: 'papaye',
  peachpuff: 'peau de pêche',
  peru: 'pérou',
  pink: 'rose',
  plum: 'prune',
  powderblue: 'bleu poudre',
  purple: 'violet',
  rebeccapurple: 'violet Rebecca',
  red: 'rouge',
  rosybrown: 'brun rosâtre',
  royalblue: 'bleu roi',
  saddlebrown: 'brun amande',
  salmon: 'saumon',
  sandybrown: 'châtain clair',
  seagreen: "vert d'eau",
  seashell: 'coquillage',
  sienna: 'sienne',
  silver: 'argent',
  skyblue: 'bleu-ciel',
  slateblue: 'bleu ardoise',
  slategray: 'gris ardoise',
  slategrey: 'gris ardoise',
  snow: 'neige',
  springgreen: 'vert printanier',
  steelblue: 'bleu acier',
  tan: 'cuir',
  teal: 'vert sarcelle',
  thistle: 'chardon',
  tomato: 'tomate',
  turquoise: 'turquoise',
  violet: 'mauve',
  wheat: 'blé',
  white: 'blanc',
  whitesmoke: 'fumée blanche',
  yellow: 'jaune',
  yellowgreen: 'jaune-vert',
} as Record<string, string>
